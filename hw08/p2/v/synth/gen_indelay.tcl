...
set dest [open indelay.tcl w]

set hrdpath [get_timing_paths -to hrdata]
puts $dest "set_input_delay [get_attribute $hrdpath arrival] -clock \$clkname \[get_port HRDATA\] "
set readypath [get_timing_paths -to hready]
puts $dest "set_input_delay [get_attribute $readypath arrival] -clock \$clkname \[get_port HREADY\] "
set resppath [get_timing_paths -to hresp]
puts $dest "set_input_delay [get_attribute $resppath arrival] -clock \$clkname \[get_port HRESP\] "
close $dest

set_driving_cell -lib_cell "$DR_CELL_NAME" -pin "$DR_CELL_PIN" [remove_from_collection [all_inputs] hclk]
...
puts "gen_indelay.tcl completed successfully (elapsed time: $timestr actual)"
exit
