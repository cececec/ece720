...
, unsigned int          read_write_loops      // number iterations through read & write loops 
, unsigned int          num_writes            // number of writes in write loop
, unsigned int          num_reads             // number of reads in read loop
)
...
, m_read_write_loops  ( read_write_loops  )     /// number iterations through read & write loops 
, m_num_writes        ( num_writes        )     /// number of writes in write loop
, m_num_reads         ( num_reads         )     /// number of reads in read loop
, m_check_all         ( true              )
{ 
...
)
{
  ...
  for (unsigned int i = 0; i < m_read_write_loops; i++ )
  {
    ...
    for (unsigned int j = 0; j < m_num_writes; j++ )
    {
      if(!m_transaction_queue.is_empty())
      {
        ...
        *reinterpret_cast<unsigned int*>(data_buffer_ptr) = w_data;
        ...
        msg.str ("");
        msg << "Initiator: " << m_ID << " Creating transaction: " 
            << "request_out_port->write (" << transaction_ptr << ")";
        REPORT_INFO(filename, __FUNCTION__, msg.str());
        report::print(m_ID, *transaction_ptr, filename, __FUNCTION__);
      }
    ...
    for (unsigned int i = 0; i < m_num_reads; i++ )
    {
      if(!m_transaction_queue.is_empty())
      {
        transaction_ptr = m_transaction_queue.dequeue ();
