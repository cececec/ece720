...
volatile unsigned char *console = (volatile unsigned char *) 0x20000000U;
...
unsigned long *dma_cr = (unsigned long *) 0x30000000U;
unsigned long *dma_sa = (unsigned long *) 0x30000004U;
unsigned long *dma_da = (unsigned long *) 0x30000008U;
unsigned long *dma_ln = (unsigned long *) 0x3000000CU;

int main(void) {
  unsigned long i, src[LENGTH], dest[LENGTH];

  printf("starting...\n");

  // Load the source array
  for(i=0; i<LENGTH/4; i++) {
    src[i]=i; // Set data equal to address
  }
  ...
  //*(dma_sa)=(unsigned long)src;
  //*(dma_da)=(unsigned long)dest;
  //*(dma_ln)=(unsigned long)LENGTH;
  //*(dma_cr)=1;
  ...
  for(i=0; i<LENGTH/4; i++) {
    if (src[i] != dest[i]) {
      printf("ERROR: Contents don't match: %x !=  %x\n",src,dest);
    }
  }
